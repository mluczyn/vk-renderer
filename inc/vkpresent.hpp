...
namespace vw {
    ...
    public:
        ...
        inline int untilClosed(std::function<void()> loop) const {
            while (!shouldClose()) {
                ...
                //std::cout << "Delta: " << std::chrono::duration_cast<std::chrono::microseconds>(deltaTime).count() << std::endl;
                glfwPollEvents();
            }
            return 0;
        }
        ...
        void setInputHandler(InputHandlerType& inputHandler) const {
    ...
    public:
        ...
        void present(uint32_t imageIndex, ArrayProxy<vk::Semaphore> waitConditions);
        void presentAndSync(uint32_t imageIndex, ArrayProxy<vk::Semaphore> waitConditions);
